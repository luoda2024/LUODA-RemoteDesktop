// https://github.com/luoda/luoda/discussions/6444#discussioncomment-9010062
...
//*************************************************************
//
//  DeleteLUODATestCertsW_SingleHive()
//
//  Purpose:    Deletes LUODA Test certificates and wrong key stores
//
...
BOOL DeleteLUODATestCertsW_SingleHive(HKEY RootKey, LPWSTR Prefix = NULL) {
...
extern "C" void DeleteLUODATestCertsW() {
	// Current user
	std::wcout << "*** Current User" << std::endl;
	DeleteLUODATestCertsW_SingleHive(HKEY_CURRENT_USER);

	// Local machine (requires admin rights)
	std::wcout << "*** Local Machine" << std::endl;
	DeleteLUODATestCertsW_SingleHive(HKEY_LOCAL_MACHINE);
	...
	for (DWORD Index = 0; ; Index++) {
		...
		DeleteLUODATestCertsW_SingleHive(HKEY_USERS, SubKeyName);
	}
